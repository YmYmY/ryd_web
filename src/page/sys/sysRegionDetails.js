    ...
    methods: {
        ...
        async doQuerySysStaticData () {
            ...
            if(that.obj.regionId > 0){
                ...
                let regionList = await that.common.postUrl("api/sysRegionBO.ajax?cmd=getSysRegionAll",{});
            ...
            //租户信息
            if(that.obj.tenantId > 0 && that.obj.regionType==2){
                let sys = await that.common.postUrl("api/sysTenantDefBO.ajax?cmd=getSysTenantDefDetails",{"tenantId":that.obj.tenantId});
                that.sys = sys;
                that.sys.tenantType = sys.tenantType+"";
                if(that.sys.tenantType == "1"){
                    that.sys.showTenant=true;
                }else {
                    that.sys.showTenant=false;
                }
                that.sys.tenantStatus = sys.tenantStatus+"";
                that.sys.tenantPermission = sys.tenantPermission+"";
                that.sys.tenantCost = sys.tenantCost / 100;
                that.sys.approvalRemarks ="";
                that.sys.pTenantName = sys.pTenantName;
                that.$refs.cardId.initDate(sys.cardId);
                that.$refs.businessLicense.initDate(sys.businessLicense);
                that.$refs.tenantLogo.initDate(sys.tenantLogo);
                that.showSysTenant=true;
            }
        },
        //区域类型发生改变
