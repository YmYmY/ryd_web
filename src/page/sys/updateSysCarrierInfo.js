...
export default {
    ...
    methods: {
        ...
        async doQuerySysStaticData () {
            ...
            let data = await that.common.postUrl("api/sysTenantDefBO.ajax?cmd=getSysCarrierInfo",{"id":that.id});
            that.sysCarrierInfo = data;
            that.obj.id = that.sysCarrierInfo.id;
            that.obj.carrierId = that.sysCarrierInfo.carrierId + "";
            that.obj.remark = that.sysCarrierInfo.remark;
            if(that.obj.carrierId == "1"){
                that.obj.tenantFullName = that.sysCarrierInfo.clientTenantId;
                that.obj.supplierFullName = that.sysCarrierInfo.supplierTenantId;
                that.obj.clientTenantId = that.sysCarrierInfo.clientTenantId;
                that.obj.supplierTenantId = that.sysCarrierInfo.supplierTenantId;
            }
            if(that.obj.carrierId == "2"){
                that.obj.tenantFullName = that.sysCarrierInfo.clientTenantId;
                that.obj.supplierFullName = that.sysCarrierInfo.supplierTenantId;
                that.obj.clientTenantId = that.sysCarrierInfo.clientTenantId;
                that.obj.supplierTenantId = that.sysCarrierInfo.supplierTenantId;
                that.obj.regionGrade = that.sysCarrierInfo.regionGrade+"";
                that.$refs.city.initData(that.sysCarrierInfo.provinceId, that.sysCarrierInfo.cityId, that.sysCarrierInfo.districtId, null);
            }
            if(that.obj.carrierId == "3"){
                that.obj.tenantFullName = that.sysCarrierInfo.clientTenantId;
                that.obj.clientTenantId = that.sysCarrierInfo.clientTenantId;
                that.obj.allocateCarrierId = that.sysCarrierInfo.allocateCarrierId;
                that.obj.warehouseCarrierId = that.sysCarrierInfo.warehouseCarrierId;
                that.obj.factoryCarrierId = that.sysCarrierInfo.factoryCarrierId;
                that.obj.reverseCarrierId = that.sysCarrierInfo.reverseCarrierId;
                that.obj.retailerCarrierId = that.sysCarrierInfo.retailerCarrierId;
            }
            if(that.obj.carrierId == "4" || that.obj.carrierId == "5"){
                that.obj.tenantFullName = that.sysCarrierInfo.clientTenantId;
                that.obj.supplierFullName = that.sysCarrierInfo.supplierTenantId;
                that.obj.clientTenantId = that.sysCarrierInfo.clientTenantId;
                that.obj.supplierTenantId = that.sysCarrierInfo.supplierTenantId;
                that.obj.storeFullName = that.sysCarrierInfo.storeId;
                that.obj.storeId = that.sysCarrierInfo.storeId;
                if(!that.common.isBlank(data.clientTenantId)){
                    that.common.postUrl("api/cmWarehouseBO.ajax?cmd=doQueryStoreAll",{"tenantId":data.clientTenantId},function (data) {
                        that.storeIdList = data.items;
                    })
                }
            }
