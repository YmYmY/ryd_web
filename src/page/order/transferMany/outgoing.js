...
export default {
  ...
  methods: {
    ...
    addTransitOrderView(){
        let item = {
          urlName: "新增二次",
          urlId: "add_transit_Many_"+2,
          urlPath: "/order/transferMany/transferAdd.vue",
          urlPathName: "/billingTransitManyOrder",
          query:{},
       }
     ...
     toTransitOrderView(type){
      ...
      let item = {
        ...
        urlPath: "/order/transferMany/transferAdd.vue",
        urlPathName: "/modifyOrViewTransitManyOrder",
        query:order,
      }
      this.$emit('openTab', item);
    },
    ...
    cancelTransitOrders(){
      ...
      that.$confirm('确认取消'+tsize+"单二次中转信息?", '提示', {
    ...
    async cancelTransit(params){
      let that = this;
      let data = await that.common.postUrl("api/ordTransitOutgoingBO.ajax?cmd=cancelTransitTwo", params);
    ...
    cancelTransitBatchNum(){
      ...
      that.$confirm('确认取消批次'+params.batchNumAlias+"二次中转信息?", '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
     }).then(() => {
       that.common.postUrl("api/ordTransitOutgoingBO.ajax?cmd=cancelBatchNumTransitTwo", params,function(data){
