...
export default {
    ...
    methods: {
        ...
        doQuery:function(){
            ...
            var dateStr = that.obj.qryEtm.substring(0,10);
            dateStr = dateStr.replace(/-/g,'');
            var date = (that.common.formatTime(new Date(),"yyyyMMdd"));
            if(Number(dateStr) >= Number(date)){
                that.$message.error('结束时间只能是昨天之前！');
                return;
            }
            ...
            that.common.postUrl("api/sysVehicleBO.ajax?cmd=getHistoricalTrack", that.obj,function(data){
